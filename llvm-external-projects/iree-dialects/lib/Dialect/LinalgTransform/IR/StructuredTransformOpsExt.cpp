...
static DiagnosedSilenceableFailure
testPadMatcher(transform_ext::MatchCallbackResult &res, Location loc,
               const mlir::transform::TransformState &state,
               ValueRange handles) {
  if (handles.size() != 1 ||
      !llvm::hasSingleElement(state.getPayloadOps(handles[0]))) {
  ...
  auto opMatcher = transform_ext::m_Operation<tensor::PadOp>(matcherContext);
...
DiagnosedSilenceableFailure transform_ext::RegisterMatchCallbacksOp::apply(
  ...
  registry.registerCallback("_test_pad_matcher", testPadMatcher);
